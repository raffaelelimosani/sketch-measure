...
//Pseudo-localization for international design: ability for designers to see impact of longer and taller strings in UI.
//
//Pseudo-localization is the process of changing text (usually English) into a fake-English, still readable, using particular cha-
//racters-mapping (e.g. a --> à,á,â,ã,ä,å,ā,ă,ą,ǻ,ά,α,а,д,..) [https://en.wikipedia.org/wiki/Pseudolocalization].
//Text in languages other than English is very often longer (~Europe) and taller (~Asia), hence once the UI is localized users may 
//see truncations and clipping, or the translations must be shortened or cut, producing bad experience outside of English-speaking
//countries. 
//Usually pseudo-loc is used by engineers in the testing-phase, but it may be too late and re-development (or re-design) is needed:
//in contrast, by using this feature designers can see the possible impact of taller and longer strings, hence re-adjust the UI im-
//mediately and produce red-lines already "world-ready", thus increasing agility in release and decreasing localization-costs. 
...
com.utom.extend({
    ...
    createRandomString: function (characterCount, unicodeStart, unicodeEnd) {
        var s = '';
        while (s.length < characterCount) s += this.getRandomChar(unicodeStart, unicodeEnd);
        return s;
    },

    getRandomChar: function (unicodeStart, unicodeEnd) {
        var n = Math.floor(Math.random() * (unicodeEnd - unicodeStart));
        return String.fromCharCode(n + unicodeStart);
    }
},
});
